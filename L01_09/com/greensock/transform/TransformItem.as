 ...
 */
	public class TransformItem extends EventDispatcher {
		/** @private **/
		public static const VERSION:Number = 1.96;
		...
		protected function init($targetObject:DisplayObject, $vars:Object):void {
			...
			if ($vars.manualBoundsOffset is Rectangle) {
				this.manualBoundsOffset = $vars.manualBoundsOffset;
			}
			_baseRect = this.getBounds(_targetObject);
			_allowDelete = setDefault($vars.allowDelete, false);
			this.constrainScale = setDefault($vars.constrainScale, false);
			...
			this.minWidth = setDefault($vars.minWidth, 1);
			this.minHeight = setDefault($vars.minHeight, 1);
			this.maxWidth = setDefault($vars.maxWidth, Infinity);
			this.maxHeight = setDefault($vars.maxScaleY, Infinity);
...
//---- GENERAL -----------------------------------------------------------------------------------------
		...
		/** @private **/
		protected function getBounds(coordinateSpace:DisplayObject):Rectangle {
			if (this.manualBoundsOffset != null) {
				var m:Matrix = _targetObject.transform.matrix;
				var bounds:Rectangle;
				if (coordinateSpace == _targetObject) {
					_targetObject.transform.matrix = _normalMatrix; //removes all transforms so we can accurately measure width/height, particularly in Flex.
					bounds = new Rectangle(this.manualBoundsOffset.x, this.manualBoundsOffset.y, _targetObject.width + this.manualBoundsOffset.width, _targetObject.height + this.manualBoundsOffset.height); 
					_targetObject.transform.matrix = m; //re-applies the original transforms
					return bounds;
			...
			return _targetObject.getBounds(coordinateSpace);
		}
		...
		protected function calibrateProxy():void {
			...
			if (!_hasSelectableText) {
				var r:Rectangle = this.getBounds(_targetObject);
...
//---- MOVE --------------------------------------------------------------------------------------------
		...
		public function moveCheck($x:Number, $y:Number, $safe:Object):void { //Just checks to see if the translation will hit the bounds and edits the $safe object properties to make sure it doesn't
			if (_lockPosition) {
				...
				var r:Rectangle = this.getBounds(_targetObject.parent);
				r.offset($x, $y);
...
//---- SCALE -------------------------------------------------------------------------------------------
		...
		public function scaleCheck($safe:Object, $angle:Number, $skew:Number, $forceConstrain:Boolean=false):void { //Just checks to see if the scale will hit the bounds and edits the $safe object properties to make sure it doesn't
			if (_lockScale) {
				...
				if (this.hasSizeLimits) {
					...
					var b:Rectangle = this.getBounds(_target);
				...
				if (_bounds != null) {
					if (!_bounds.containsRect(this.getBounds(_target.parent))) {
						if ($safe.sy == 1) {
							...
							var r:Rectangle = this.getBounds(_target);
		...
		public function fitInsideBounds(move:Boolean=true, updateFirst:Boolean=true, customBounds:Rectangle=null):void {
			...
			if (_target.parent && mBounds != null) {
				var bounds:Rectangle = this.getBounds(_target.parent);
...
//---- GETTERS / SETTERS --------------------------------------------------------------------------------
		...
		public function get width():Number {
			if (_targetObject.parent != null) {
				return this.getBounds(_targetObject.parent).width;
			} else {
				var s:Sprite = new Sprite();
				s.addChild(_targetObject);
				var w:Number = this.getBounds(s).width;
		...
		public function get height():Number {
			if (_targetObject.parent != null) {
				return this.getBounds(_targetObject.parent).height;
			} else {
				var s:Sprite = new Sprite();
				s.addChild(_targetObject);
				var h:Number = this.getBounds(s).height;
				s.removeChild(_targetObject);
				return h;
			}
		}
		...
		public function set maxWidth(value:Number):void {
			...
			var m:Matrix = _targetObject.transform.matrix;
			var w:Number = this.getBounds(_targetObject).width * Math.sqrt(m.a * m.a + m.b * m.b);
			if (w > _maxWidth) {
		...
		public function set maxHeight(value:Number):void {
			...
			var m:Matrix = _targetObject.transform.matrix;
			var h:Number = this.getBounds(_targetObject).height * Math.sqrt(m.c * m.c + m.d * m.d);
			if (h > _maxHeight) {
		...
		public function set minScaleX($n:Number):void {
			if ($n == 0) {
				$n = this.getBounds(_targetObject).width || 500;
		...
		public function set minScaleY($n:Number):void {
			if ($n == 0) {
				$n = this.getBounds(_targetObject).height || 500;
		...
		public function set maxScaleX($n:Number):void {
			if ($n == 0) {
				$n = this.getBounds(_targetObject).width || 0.005;
		...
		public function set maxScaleY($n:Number):void {
			if ($n == 0) {
				$n = this.getBounds(_targetObject).height || 0.005;
